# # nlp_model/ai_engine.py
# # Main NLP engine: classify, call LLM (Groq), parse JSON, post-filter for safety.
...
#     # BROAD search (India-friendly)
#     query = f"""
# [out:json][timeout:30];
# (
#   /* Official recycling centers */
#   node["amenity"="recycling"](around:{radius},{lat},{lon});
#   way["amenity"="recycling"](around:{radius},{lat},{lon});

#   /* Scrap shops (India's common recycler tag) */
...
#     try:
#         response = requests.post(overpass_url, data=query, headers={"User-Agent": "SmartRecyclingAI/1.0"})
#         data = response.json()
...
#                 "type": tags.get("shop") or tags.get("amenity", "Recycling"),
#                 "material": tags.get("recycling:material", "Various"),
...
#     # Ask user for nearby recycling shops
#     need_shops = input("\nDo you need nearby recycling shops? (yes/no): ").strip().lower()

#     if need_shops in ["yes", "y"]:
#         user_city = input("Enter your city or location: ").strip()
#         shops = find_recycling_shops_osm(user_city)
